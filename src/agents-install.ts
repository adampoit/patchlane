...
const DEFAULT_SKILLS_BASE_URL =
  "https://raw.githubusercontent.com/adampoit/patchlane/main/skills";
...
function resolveSourceBaseUrl(ref: string) {
  ...
  return normalizeBaseUrl(
    `https://raw.githubusercontent.com/adampoit/patchlane/${ref}/skills`,
  );
}
...
function parseManifest(value: unknown): SkillManifest {
  ...
  const skills = value.skills.map((entry) => {
    if (!isPlainObject(entry))
      fail("Each skills manifest entry must be an object.");
    const name = entry.name;
    const files = entry.files;

    if (typeof name !== "string")
      fail("Skill manifest entry is missing a string name.");
...
async function fetchText(url: string) {
  ...
  if (!response.ok) {
    fail(
      `Failed to download '${url}': ${response.status} ${response.statusText}`.trim(),
    );
  }
  ...
  const files = await Promise.all(
    skill.files.map(async (relativePath) => ({
      ...
      contents: await fetchText(
        buildUrl(sourceBaseUrl, `${skill.name}/${relativePath}`),
      ),
    })),
  );
...
function readInstallState(filePath: string): InstallState {
  ...
  try {
    ...
    const managedSkills = Array.isArray(parsed.managedSkills)
      ? parsed.managedSkills.filter(
          (value): value is string => typeof value === "string",
        )
      : [];
    const sourceBaseUrl =
      typeof parsed.sourceBaseUrl === "string" ? parsed.sourceBaseUrl : "";
...
export async function installPatchlaneAgents(
  options: InstallPatchlaneAgentsOptions = {},
) {
  const installDir = path.resolve(
    process.cwd(),
    options.installDir ?? DEFAULT_INSTALL_DIR,
  );
  ...
  const fetchedSkills = await Promise.all(
    manifest.skills.map((skill) => fetchSkill(sourceBaseUrl, skill)),
  );
