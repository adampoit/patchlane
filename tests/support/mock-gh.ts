...
function saveJson(stateDir: string, name: string, value: unknown) {
	writeFileSync(path.join(stateDir, name), `${JSON.stringify(value, null, 2)}\n`);
}
...
function render(value: unknown, jq?: string) {
	let result = value;
	if (jq === '.url') result = typeof value === 'object' && value ? ((value as { url?: string }).url ?? '') : '';
	if (jq === '.[0].number')
		result = Array.isArray(value) && value.length ? (value[0] as { number: number }).number : '';
...
function getStateDir() {
	...
	if (!stateDir) fail('FORK_SYNC_GH_STATE_DIR is required for the mock gh CLI.');
	return stateDir;
}
...
function createPr(stateDir: string, argv: string[]) {
	...
	const existing = prs.find((pr) => pr.head === head && pr.base === base && pr.state === 'open');
	if (existing) {
		render(existing.url);
		return;
	}
