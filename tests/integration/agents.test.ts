...
async function startSkillsServer(state: SkillState) {
	const server = createServer((request, response) => {
		...
		const contentType = relativePath.endsWith('.json') ? 'application/json' : 'text/plain; charset=utf-8';
...
test('agents command installs and updates managed skills', async () => {
	...
	try {
		...
		expect(readFileSync(path.join(tempRoot, '.agents/skills/patchlane-fork-setup/SKILL.md'), 'utf8')).toBe(
			'setup skill v1\n',
		);
		expect(
			readFileSync(path.join(tempRoot, '.agents/skills/patchlane-fork-setup/references/checklist.md'), 'utf8'),
		).toBe('checklist v1\n');
		expect(readFileSync(path.join(tempRoot, '.agents/skills/patchlane-sync-patches/SKILL.md'), 'utf8')).toBe(
			'sync skill v1\n',
		);
		...
		writeFileSync(path.join(tempRoot, '.agents/skills/custom-skill/SKILL.md'), 'custom skill\n');
		...
		expect(existsSync(path.join(tempRoot, '.agents/skills/patchlane-fork-setup/references/checklist.md'))).toBe(
			false,
		);
		expect(existsSync(path.join(tempRoot, '.agents/skills/patchlane-sync-patches'))).toBe(false);
		expect(readFileSync(path.join(tempRoot, '.agents/skills/patchlane-maintenance/SKILL.md'), 'utf8')).toBe(
			'maintenance skill v1\n',
		);
		expect(readFileSync(path.join(tempRoot, '.agents/skills/custom-skill/SKILL.md'), 'utf8')).toBe(
			'custom skill\n',
		);
	} finally {
		await server.close();
